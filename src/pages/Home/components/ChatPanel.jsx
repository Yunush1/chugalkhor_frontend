...
import IncomingCallOverlay from "../atoms/Incomingcalloverlay";
import { useWaveCall } from "../hooks/usewavecall";
...
export default function ChatPanel({
    ...
    const { data: messages = [] } = useNewMessages(roomId, { enabled: !!roomId });
    ...
    const [callTypeChoice, setCallTypeChoice] = useState("video");

    const handleStartCall = async (type) => {
        if (!isJoined) return;
        try {
            await call.startCall(roomId, type);
        } catch (err) {
    ...
    const handleLeaveRoom = () => onLeave(room);
    ...
    return (
        <div style={{ ...S.chatPanel, position: "relative" }}>
            ...
            {/* ── Active call screen (sits on top of chat) ───────────────── */}
            {call.inCall && <CallScreen call={call} />}
            ...
            <div style={S.chatHeader}>
                ...
                <div
                    ...
                    onClick={() => openUserModal?.(room || { _id: "12" })}
                >
                    <div style={S.chatName}>{name || "Room"}</div>
                    <div style={S.chatSub}>
                        {members?.length || 0} of {maxMembers || 0} members
                ...
                {/* ── Call buttons ────────────────────────────────────────── */}
                {isJoined && !call.inCall && (
                    <div style={callBtnGroup}>
                        <button
                            style={callBtn}
                            title="Audio call"
                ...
                {/* End / leave call shortcut while in call */}
                {call.inCall && (
                    <button
                        style={{ ...callBtn, background: "rgba(239,68,68,0.15)", borderColor: "#ef4444" }}
                        title="Leave call"
                ...
                <Button
                    ...
                    className="text-red-500 border-1 border-red-500 hover:text-red-700"
                >
                    <LogoutOutlined />
                    LEAVE
                </Button>
            ...
            <div style={S.messages}>
                ...
                <div style={{ display: "flex", margin: "8px 0" }}>
                    <div style={S.bubble}>
                        <div style={S.bubbleTail} />
                        <p style={S.bubbleText}>{description}</p>
                        <hr style={{ border: "none", borderTop: `1px solid ${C.divider}`, margin: "10px 0" }} />
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                            <InfoCell label="Created by" val={`User ${(createdBy || "").slice(-6)}`} />
                            <InfoCell label="Members" val={`${members.length || 0} / ${maxMembers || 0}`} />
                        </div>
                        <div style={{ marginTop: 12 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                <span style={{ fontSize: 12, color: C.textSecond }}>Capacity</span>
                                <span style={{ fontSize: 12, fontWeight: 700, color: pct >= 90 ? "#FF6B35" : C.greenLight }}>
                                    {pct}%
                                </span>
                            </div>
                            <div style={{ height: 4, backgroundColor: C.bg, borderRadius: 2, overflow: "hidden" }}>
                                <div style={{
                                    height: "100%", borderRadius: 2, transition: "width .4s ease",
                                    width: `${pct}%`, backgroundColor: pct >= 90 ? "#FF6B35" : C.greenLight,
                                }} />
                            </div>
                        </div>
                        <span style={{ fontSize: 11, color: C.textSecond, display: "block", textAlign: "right", marginTop: 6 }}>
                            {shortTime(createdAt)}
                        </span>
                    </div>
                </div>
                ...
                {messages.map((msg) => {
                    const isOwn = msg?.sender === "me" || msg?.sender?._id === user?._id;
                    return (
                        <MessageBubble
                            key={msg._id || msg.id}
                            message={msg}
